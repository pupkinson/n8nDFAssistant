...
- **Справочник по нодам n8n:** MCP `n8n-mcp` (параметры/версии нод, справочник). Других MCP нет (второй отключён).
...
## 7.5) Branch Join Policy (Hard Rule)
...
1) В каждой ветке (TRUE и FALSE) сделай короткий `Set — Normalize branch output`, чтобы обе ветки отдавали **одинаковую структуру полей** (хотя бы ключи, которые нужны дальше).
2) Поставь узел `Merge — <topic> (passThrough)`:
   - **Mode:** `Pass-through`
   - **Output:** `Input 1`
3) Подключи **обе** ветки (TRUE и FALSE) к **одному и тому же** входу Merge: `Input 1`.
   - Это нормально: у одного входа Merge может быть несколько входящих соединений.
   - Вход `Input 2` остаётся пустым.
4) Дальше весь core pipeline подключай **к выходу Merge**.

Что это гарантирует:
- downstream ноды не получат 2 входящих соединения,
- workflow не будет ждать «вторую ветку», которой не было.

#### B) Join параллельных веток, которые обязаны обе выполниться (parallel)

Цель: собрать результаты двух веток в один item.

1) Ветка A формирует «anchor» (ctx/req/keys) → в `Merge Input 1`.
2) Ветка B делает I/O и кладёт результат под неймспейс (`db.*`, `http.*`) → в `Merge Input 2`.
