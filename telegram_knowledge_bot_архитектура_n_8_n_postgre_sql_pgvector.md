...
- **Какие источники может использовать:** только информация, ранее опубликованная **в этом же чате (`chat_id`)**. Запросы «принеси из другого чата/лички» в группе — отказ.
...
### 2.6.2 Group (публичные группы/чаты)
...
- Branch-join hard rule: после IF/Switch запрещены множественные входы в обычные ноды (Set/IF/Postgres/HTTP/Telegram). Join веток — только через Merge (с корректным mode и Notes).
- Используем MCP `n8n-mcp` (справочник нод/параметров). Второй MCP отключён.
